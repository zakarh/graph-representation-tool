...
class NodePartitionManager(object):
    def __init__(
        ...
        # }
        #     self.np: NodePartition = NodePartition(**args)
        self.data = {}
    ...
    def create(self, key, properties=None) -> bool:
        try:
            ...
            # elif id(self.np) != id(np):
            elif np.size < self.capacity:
                # print(np.size, np.capacity)
                # self.commit()
                np = np
                np.nodes[key] = properties
                np.size += 1
...
class EdgePartitionManager(object):
    ...
    def create(self, source, target, properties=None) -> bool:
        try:
            ...
            if erp is None:
                ...
                self.erp.dump()
                self.erp = erp
            # elif id(self.erp) != id(erp):
            #     # self.commit_erp()
            #     self.erp = erp
            #     if source in self.erp.relations["out"]:
            #         if target in self.erp.relations["out"][source]:
            #             return False
            #     self.erp.relations["out"].setdefault(source, set())
            #     self.erp.relations["in"].setdefault(target, set())
            #     self.erp.relations["out"][source].add(target)
            #     self.erp.relations["in"][target].add(source)
            #     self.erp.size += 1
            #     self.erp.dump()
            else:
                if source in self.erp.relations["out"]:
            ...
            if epp is None:
                ...
                self.epp = epp
                self.epp.dump()
            # elif id(self.epp) != id(epp):
            #     # self.commit_epp()
            #     self.epp = epp
            #     self.epp.properties.setdefault(source, {})
            #     self.epp.properties[source][target] = properties
            #     self.epp.size += 1
            #     self.epp.dump()
            else:
                self.epp = epp
    ...
    def delete(self, source, target, check_existence=True, check_memory=True) -> bool:
        # if check_memory:
        #     if check_existence:
        ...
        #         if source in self.erp.relations["out"]:
        #             if target in self.erp.relations["out"][source]:
        #                 self.erp.relations["out"][source].remove(target)
        #                 self.erp.relations["in"][target].remove(source)

        #                 if len(self.erp.relations["out"][source]) == 0:
        #                     del self.erp.relations["out"][source]
        #                 if len(self.erp.relations["in"][target]) == 0:
        #                     del self.erp.relations["in"][target]

        #                 self.erp.size -= 1
        #                 edge_relationship_deleted = True
        #         if source in self.epp.properties:
        #             if target in self.epp.properties[source]:
        #                 self.epp.size -= 1
        #                 del self.epp.properties[source][target]
        #                 if len(self.epp.properties[source]) == 0:
        #                     del self.epp.properties[source]
        #                 edge_value_deleted = True
        #         if edge_relationship_deleted and edge_value_deleted:
        #             return True
        #     else:
        #         self.erp.relations["out"][source].remove(target)
        #         self.erp.relations["in"][target].remove(source)
                ...
                if source in erp.relations["out"]:
                    if target in erp.relations["out"][source]:
                        ...
                        if source in self.erp.relations["out"]:
                            if len(self.erp.relations["out"][source]) == 0:
                                del self.erp.relations["out"][source]
                        if target in self.erp.relations["in"]:
                            if len(self.erp.relations["in"][target]) == 0:
                                del self.erp.relations["in"][target]
    ...
    def contains(self, source, target, check_memory=True) -> bool:
        if check_memory:
            if source in self.erp.relations["out"]:
                if target in self.erp.relations["out"][source]:
                    return True
    ...
    def partition_for_create_edge_property(
        ...
        if buckets == self.epp.bucket:
            if self.epp.size < self.capacity:
                return self.epp, True
        args = {
            "directory": self.directory.joinpath("properties"),
            "bucket": buckets,
            "capacity": self.capacity,
