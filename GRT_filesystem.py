...
class EdgeManager:
    ...
    def incoming(self, node_id):
        return [
            f.split(self.sep)[0]
            for f in os.listdir(self.storage_dir)
            if f.endswith(".json") and f.split(self.sep)[1].replace(".json", "") == node_id
        ]
    ...
    def outgoing(self, node_id):
        return [
            ...
            for f in os.listdir(self.storage_dir)
            if f.endswith(".json") and f.split(self.sep)[0] == node_id
        ]
...
class NodeManager:
    ...
    def create(self, node_id, properties=None):
        if properties is None:
            properties = {}
    ...
    def delete(self, node_id):
        ...
        if os.path.exists(node_path):
            os.remove(node_path)
        for into in self.edges.incoming(node_id):
            self.edges.delete(into, node_id)
        for outfrom in self.edges.outgoing(node_id):
            self.edges.delete(node_id, outfrom)

    def contains(self, node_id):
        return os.path.exists(self._node_path(node_id))
